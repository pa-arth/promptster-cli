...
- feat(hosted): the CLI stops looking for a GitHub Codespace, because there is no longer one to find. `hosted` now means a fenced E2B box Promptster provisions, seeds and pays for (openspec `private-problem-sandbox-lane` 2.4i; backend #852, hire #42, cli #40), and every GitHub-specific mechanism the old lane needed is gone with it: the `CODESPACES`/`CODESPACE_NAME` probe, `--adopt` auto-enabling from the environment, `gh codespace delete` and its wind-down print, `abort --delete-codespace`, `doctor`'s hosted section, and the whole boot reporter that read creation time and prebuild state off GitHub's codespaces API with an ambient `GITHUB_TOKEN`. That reporter has no successor here on purpose — a CLI inside a box cannot observe when the box was created, so the worker that provisioned it files that report instead. What survives is everything that was never about the host: `--seeded` still adopts the checkout it is sitting in and still verifies it by TREE rather than by remote, because a remote is a claim about where bytes came from and the question is what bytes are here
- fix(submit): the audit-trail submission commit is made on every lane again. It was suppressed on the hosted lane as a PRIVACY mechanism, not a tidy-up: committing from a read-only Codespace makes GitHub create a public fork under the candidate's own account — a repository named after the assessment problem, sitting on their profile, disclosing their job search to their current employer. Suppressing our own commit was the one half of that we controlled. A provisioned box has no `origin` we do not control and nothing that forks on commit, so the hazard left with the host and the suppression would now be costing the commit on the lane that will carry most assessments in exchange for nothing. The commit was always best-effort and still is — the bundle reads the index and the diff compares the working tree to the base, so neither has ever depended on a commit existing
- fix(done): `promptster done` no longer promises to delete a machine it cannot delete. The hosted brief told candidates it "submits your work and then deletes this codespace", which was true and load-bearing when the machine was theirs and a merely-stopped codespace kept billing their storage allowance for 30 days. The box is ours and the provisioner pauses it, so `done` now says the machine is ours and shuts down on its own. A promise the binary has quietly stopped keeping is worse than no promise: the candidate who believes it goes looking for something to clean up and finds nothing. 1.10.2's expiry fix is unaffected — `uploadConfirmed` still stays false on the already-closed path; it simply no longer has an irreversible teardown to gate, and the rule that `done` must not say "submitted" for work that did not reach us stands on its own
- fix(doctor): the "system shell init already sources the hook" pass is keyed on the session rather than the environment. It used to require `CODESPACES`, and there is no box-side variable to swap in for it — the session file the provisioner wrote is what actually implies the image did the RC injection, and unlike an environment variable it survives a `done` or `doctor` run in a bare `sh -c` that inherited nothing. The same reasoning replaced `hostedLaneActive` (a session flag ORed over an environment probe) with `seededSession`, which reads only the session: the old predicate needed the flag precisely BECAUSE it also consulted an environment that could vanish

## 1.10.2 - 2026-08-27

- fix(expiry): the client only ever learned its deadline from a hook handler, so it noticed the time limit when the agent next spoke — minutes later for a candidate reading code, never for one who had stopped typing. The server's timer closes the key on schedule and every upload route checks it, so the server won the ordinary race and the candidate's last work was locked out before it could be delivered. `promptster diff-watch` now owns the clock — it runs for every session and lane, re-reads the session file each poll, and sleeps to the deadline instead of past it (detection ~0s at the deadline, 60s worst case), with a forced snapshot 90s before the flip so the common case delivers bytes before it's needed. An expiry marker written before the spawn stops the old loop where every post-deadline hook spawned another concurrent `promptster done`. The expiry banner and results URL, previously landing only in a log file nobody opens, are now delivered through a one-shot global-dir notice hooks drain and print. `uploadConfirmed` stays false on the already-closed path, so codespace teardown can't destroy the only copy of unreceived work
- fix(codex): the codex rail reported zero clears or compactions, ever — the assumption on record was that codex had no compact concept, which was never true. The rollout file this normalizer already walks writes a `context_compacted` event on every compaction; nothing read it. A candidate was docked for a ritual our own capture could not see (`context_hygiene [developing] — zero clears or compactions`) despite genuinely resetting context. Keyed off the event marker rather than its sibling record, which carries the full prior conversation (base64 images included) and would be the wrong thing to capture. No trigger or token-percent fields are invented — codex's rollout carries neither
- fix(codex): unwrapped the 0.149+ `exec` tool, whose input is a small JS program calling the real tool — every call was landing as `toolName="exec"` with the fluency judge seeing zero commands and zero diffs. Two more shapes still lost the command text after that first fix: a whole-program regex couldn't match the quoted-key form of `cmd:` (dropping 14.5% of exec_command calls on a 1,160-call corpus), and the same scan misread batch loops, reporting only the first job of a batch. Now reads the literal belonging to the actual call site, skipping string/comment decoys, with an escape-aware scanner instead of a fallback regex. Combined loss on the same corpus: 8.4% → 1.6%
- fix(hook): the wrapped `codex` function called itself the second time the shell hook was sourced — `command -v codex` returns our own function's name on re-source, that answer went straight into the "run the real binary" branch, and zsh aborted on nested-function depth while bash hung. Resolved through a scratch variable so only an absolute path is ever committed
- fix(codex): the normalizer now emits interrupts and web lookups that the rollout already reports — `turn_aborted` events (an unanswered function call, plus whether the following prompt follows an interrupt) and `web_search_end` events (including the query-less page-fetch shape). Verified against 41 local rollouts: 3 interrupts and 6 web lookups the normalizer had been dropping entirely

## 1.10.1 - 2026-08-26

- chore(vsix): the bundled editor extension moves to promptster-vscode 0.3.2, which stops shipping our repository machinery into a candidate's editor. The 0.3.1 artifact carried `.github/workflows/ci.yml`, `RELEASING.md` and `pnpm-lock.yaml` inside the installed extension; 0.3.2 excludes those plus `scripts/`, `dist-vsix/` and the emitted `.d.ts` declarations, leaving `package.json`, `README.md`, `CHANGELOG.md`, `.promptsterignore.default` and the icon as the only non-`dist/` files. **Nothing the extension collects or emits changes** — `git diff v0.3.1..v0.3.2` touches no `src/`, so the compiled payload is identical. This release exists because the CLI is the only distribution path there is: `promptster start` sideloads the embedded `.vsix` on every run and there is no marketplace auto-update, so an extension fix that is not embedded and released here reaches nobody. Pinned by sha256 `94bb2036…` against tag `v0.3.2`, and `vsix/embedded_test.go` asserts the bytes and the pin agree

## 1.10.0 - 2026-08-26

- fix(capture): codex ≥0.149 sessions record the conversation again. Codex 0.149 replaced the per-message rollout lines this normalizer was built against — `event_msg/user_message` and `event_msg/agent_message` are gone, and every item now arrives as `event_msg/item_completed` with its text under `item.content[].text` and the agent phase under `item.phase`. Nothing recognised that shape, so on every current codex build a captured session contained `session_start` and `tool_use` and NOT ONE prompt or answer: the session looked captured, the reviewer opened it, and the conversation was simply absent. Both shapes are handled now, so an older codex keeps working. The messages are read from `item_completed` rather than from `response_item`, which carries the same conversation plus codex's developer-role system preamble and its `<recommended_plugins>` boilerplate as `role="user"` — capturing those as candidate prompts would be worse than capturing nothing
- fix(capture): a watcher left running by a PREVIOUS session no longer stops the new one from ever starting. A watcher resolves its session, its workspace root and its start cutoff once at boot and never again, while the state file it heartbeats into is resolved per write through `~/.promptster/active-workspace`. So the moment a new `start` moved that pointer, last session's watcher began stamping the NEW session's state file, `ensureCodexWatcher` saw a live PID and declined to launch, and the only watcher running was one matching every rollout against the PREVIOUS workspace — classifying all of it "no match". A candidate could work for hours in a correctly configured workspace and reach a reviewer as someone who did nothing, with no error anywhere. Found in the field on a watcher that had been running since two sessions earlier. All three daemons (codex rollout, Claude transcript, git diff) now stamp the session they were booted for, `start` replaces one belonging to a different session, and a watcher whose session stops being active stands down rather than outliving it. A watcher standing down does not delete the incoming watcher's state or progress
- feat(status): `promptster status` reports codex capture health and stops claiming Claude hooks on a codex-only session. Capture's failure mode is silence, so "watching (pid N, K events sent)" versus "NOT being recorded" is the one line that would have made the bug above visible from the candidate's side on day one
