...
// ran verify-cli. That is still a self-attestation — it is not a substitute
...
//   4. A verify-cli attestation from a trusted author names THIS head SHA.
...
export const REQUIRED_CHECKS = [
  ...
  },
  {
    name: 'install-check',
    appSlug: 'github-actions',
    workflowPath: '.github/workflows/packaging.yml',
  },
  {
...
// An agent posts this as a PR comment after it has driven verify-cli.
...
export const VERIFICATION_MARKER =
  /<!--\s*verified:\s*verify-cli\s+sha=([0-9a-f]{7,40})\s*-->/i;
...
export function shouldAutomerge(input) {
  ...
  if (input.isDraft) {
    return {
      merge: false,
      reason: 'draft — verification gate; mark ready only after verify-cli proof',
    };
  }
  ...
  return {
    merge: true,
    reason:
      'ready, trusted checks green, Greptile pass, verify-cli attested for this SHA, no privileged-path edits',
  };
}
...
export function verificationBlocksMerge({
  ...
  if (!claimed.length) {
    if (untrusted) {
      return `the only verify-cli attestation on this PR is from an untrusted author — anyone can comment, so only ${trustedAssociations.join('/')} counts`;
    }
    return 'no verify-cli attestation on this PR — run verify-cli and post one';
  }
  ...
  if (!matched) {
    return `verify-cli attestation is for ${claimed.join(', ')}, not head ${head.slice(0, 8)} — re-verify after the last push`;
  }
  return null;
}
