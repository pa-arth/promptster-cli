...
const attested = (sha = HEAD_SHA, authorAssociation = 'OWNER') => [
  { body: `<!-- verified: verify-cli sha=${sha} -->\nDrove it.`, authorAssociation },
];
...
test('refuses when the PR edits a privileged workflow or automerge script', () => {
  const r = decide({ changedFiles: ['.github/workflows/packaging.yml', 'README.md'] });
  assert.equal(r.merge, false);
  assert.match(r.reason, /privileged path/);
  assert.ok(r.reason.includes('.github/workflows/packaging.yml'), r.reason);
});
...
test('refuses a PR with no verify-cli attestation', () => {
  const r = decide({ comments: [said('nice work'), said('<!-- greptile_summary --> ...')] });
  assert.equal(r.merge, false);
  assert.match(r.reason, /no verify-cli attestation/);
});
...
test('REQUIRED_CHECKS stay in lockstep with this repo\'s real workflow jobs', async () => {
  ...
  const wf_packaging_yml = wf('.github/workflows/packaging.yml');
  const wf_verify_map_yml = wf('.github/workflows/verify-map.yml');

  assert.match(wf_ci_yml, /^  test:$/m);
  assert.match(wf_packaging_yml, /^  install-check:$/m);
  ...
  assert.equal(REQUIRED_CHECKS[1].workflowPath, '.github/workflows/packaging.yml');
  assert.equal(REQUIRED_CHECKS[2].appSlug, 'github-actions');
  assert.equal(REQUIRED_CHECKS[2].workflowPath, '.github/workflows/verify-map.yml');
  assert.equal(REQUIRED_CHECKS[3].appSlug, 'greptile-apps');
  assert.equal(REQUIRED_CHECKS[3].workflowPath, undefined);
  ...
  assert.match(wf_packaging_yml, /automerge-decision\.test\.mjs/);
...
test("the automerge workflow runs main's copy of the script, not the PR's", async () => {
  ...
  assert.match(
    wf,
    /contains\(github\.event\.comment\.body, 'verified: verify-cli'\)/,
