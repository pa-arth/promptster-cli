...
func codexWatcherStatePath() string { return filepath.Join(stateDir(), "codex-watcher.json") }
func codexWatcherLogPath() string   { return filepath.Join(stateDir(), "codex-watcher.log") }
...
func codexWatchProgressPath() string { return filepath.Join(stateDir(), "codex-watcher-progress.json") }
...
func startCodexWatcherProcess() error {
	...
	cmd := exec.Command(promptsterBin(), "codex-watch")
	cmd.Stdin = devNull
	cmd.Stdout = logFile
	cmd.Stderr = logFile
	return cmd.Start()
...
func runCodexWatcher() error {
	...
	if state, ok := isCodexWatcherRunning(); ok && state.PID != os.Getpid() {
		return fmt.Errorf("codex watcher already running (pid %d)", state.PID)
	}
	...
	// Only consider rollout sessions that started at/after this assessment began,
	// so we never replay the candidate's unrelated prior codex sessions.
	...
	processors := map[string]*codexRolloutProcessor{}
	eventsSent := 0
...
func pollCodexRollouts(
	...
	_ = filepath.Walk(dir, func(path string, info os.FileInfo, err error) error {
		...
		// Skip files last modified before the assessment started.
		...
		if proc == nil {
			proc = newCodexRolloutProcessor(session.SessionID, session.ConsentToIntegrity)
...
// enough to belong to this assessment.
...
func tailCodexRollout(
	...
	for {
		...
		for _, event := range proc.process(redacted) {
			...
			// Advance the decision-point counter so the explain nudge fires on
			// codex sessions too (parity with the Claude hook path). The nudge
			// itself still needs a delivery channel for codex — see memory
			// project_unified_explain_recorder — but the detection is shared.
