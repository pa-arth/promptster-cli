...
type Actor struct {
	...
	Role string `json:"role,omitempty"` // assistant | candidate | session
}

func humanActor() *Actor  { return &Actor{Type: "human", Role: "candidate"} }
...
func normalizeClaudeCode(payload map[string]interface{}, sessionID string) (Event, bool) {
	...
	case "UserPromptSubmit":
		...
		// Cadence enrichment — only when the candidate opted in to integrity checks.
		// These fields help the backend flag pasted-vs-typed work without capturing
		// keystrokes. Fields are additive and gracefully absent for older candidates.
		if saved, err := loadSession(); err == nil && saved.ConsentToIntegrity {
			length := len(promptText)
			data["promptLengthChars"] = length
			if last := loadLastPromptTs(); !last.IsZero() {
				deltaMs := time.Since(last).Milliseconds()
				data["timeSinceLastPromptMs"] = deltaMs
				if length > 300 && deltaMs < 1000 {
	...
	case "SessionStart":
		...
		// Carry session-level integrity metadata through the timeline so the
		// worker's session-integrity detector can read it without a separate API.
		if saved, err := loadSession(); err == nil {
			if saved.ConsentToIntegrity {
				data["consentToIntegrity"] = true
			}
			if saved.HoneypotToken != "" {
				data["honeypotToken"] = saved.HoneypotToken
			}
		}
		e.Data = data
		e.RawPayload = raw
